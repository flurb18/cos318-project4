...
// This file catches irq1 and read data from the keyboard controller and converts
// the data into ascii and buffers the character
...
// Assume Q is pressed on the keyboard, then the scancode for Q is 0x10
// scan_to_ascii[10] = {0x71,0x51,0x11,normal_handler}
// The associated handler is normal_handler()
// 0x71 = ASCII(q), 0x51 = ASCII(Q), 0x11 = ASCII(something)
// The keyboard codes can found in The Undocumented PC, Page 332
...
void normal_handler(unsigned char scan) {
  ...
  if (key_release == FALSE) {
    ...
    // Check shift_status to decide what to return in the character buffer
    // NOTE: This will not work if two or more "shift" keys are pressed and so
    // Caps Lock + Shift + K returns ASCII(0) not 'k'
    switch (shift_status) {
    case LEFT_SHIFT:
    case RIGHT_SHIFT:
    case CAPS_SHIFT:
...
// This function is called when a keyboard interrupt arrives (See The
// Undocumented PC, Page 332)
void keyboard_interrupt(void) {
  ...
  // Mask of the release bit (set bit 7 to zero)
  key &= 0x7f;
...
int do_getchar() {
  char buf;
  ASSERT(!disable_count);
  do_mbox_recv(keyboard_mbox, &buf, sizeof(char));
  return (int)buf;
}

// Called by the keyboard interrupt (irq1) handler (through normal_handler) to
// insert an ascii character in the keyboard queue
static void putchar(struct character *c) {
  (void) c;
  // TODO: Fill this in
  leave_critical();
  do_mbox_send(keyboard_mbox, (char *)&c->character, sizeof(char));
  enter_critical();
}
