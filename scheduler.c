...
void release_locks(pcb_t *task) {
  ASSERT(disable_count);
  // Lock releasing handles removal from the queue
  while (!queue_empty(&task->lock_queue))
    lock_release_helper( (lock_t *)queue_first(&task->lock_queue) );
}
